...
export const HomeScreenClient: React.FunctionComponent<HomeScreenClientProps> = observer(props => {
  ...
  const menu = menuList.map((el, i) => {
    ...
    if (el.onClick === "link") {
      onPress = () => {
        Linking.openURL("https://strongby.science/more_info_mitko")
      }
    }
  ...
  return (
    ...
    >
      ...
      >
        <ProgressCircle
          percent={
            (rootStore.numberOfTrainingsForLoggedClientForActiveCards.counterDone /
              rootStore.numberOfTrainingsForLoggedClientForActiveCards.counterTotal) *
              100 || 0
          }
          radius={100}
          borderWidth={15}
        ...
        >
          ...
          >
            ...
            >
              {translate("clientHomeScreen.progressCircleTextTop")}
            </Text>
            <Text
              style={{
                color: "#666666",
            ...
            >
              {translate("clientHomeScreen.progressCircleTextBottom") + " ХХХ"}
            </Text>
          </View>
        </ProgressCircle>
      </View>
      ...
      >
        {menu}
        {rootStore.loggedUser.item.isTrainer && (
          <ButtonSquare
            title={"Trainer menu"}
            onPress={() => navigation.navigate("home_tr")}
            // leftIcon={el.iconLeft}
            rightIcon={"gr_arrow_forward"}
          ></ButtonSquare>
        )}
      </View>
    </Screen>
