...
export const CardStoreModel2 = types
  .model("RootStore")
  .props({
  ...
  .actions(self => ({
    ...
    async removeTrainingYesterday(card) {
      const pos = card.item.visits.indexOf(
        moment()
          .subtract(1, "days")
          .format("MMM DD[,] YY")
          .toString(),
      ...
      pos > -1
        ? card.item.visits.splice(pos, 1)
        : console.log("Client trained yesterday, but date is not in visits")
  ...
  .views(self => ({
    isActiveCard(cardId: string): boolean {
      ...
      if (cardy.item.card_type.type === card_types.monthly) {
        const cardyExpiryDate = +moment(cardy.item.datestampStart).add(
          cardy.item.card_type.card_limit,
          "M",
        )
    ...
    getYouClientsEmails(email: string) {
      let newArr = self.cards.filter(card => card.item.trainers.includes(email))
      let newArr2 = []
      newArr.map((item, index) => item.item.clients.map(client => newArr2.push(client)))
      return newArr2
    },
  }))
