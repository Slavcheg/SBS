...
export const UserStoreModel2 = types
  .model("RootStore")
  .props({
  ...
  .actions(self => firebaseFuncs<IUser2>(self.refreshItems, self.collection))
  .actions(self => ({
    async addToDiary(userEmail: string, newDiaryEntry: IDiaryItem) {
      let user = self.users.find(user => user.item.email === userEmail)
      user.item.diary.push(newDiaryEntry)
      self.updateItem(user.id, getSnapshot(user.item)).then(res => {
        console.log(res)
      })
    },
      ...
      try {
        return (
          self.users
            ...
            .find(
              user =>
                user.item.client.generic_number == gen_num && user.item.client.password == password,
            ).item.email || null
        )
      } catch {
        return null
    ...
    isUserExistend(googleProfile: any): boolean {
      let u = self.users.find(user => user.item.email == googleProfile.email)
      if (u) {
  ...
  .views(self => ({
    ...
    getUserIDByEmail(email) {
      let user = self.users.find(user => user.item.email === email)
